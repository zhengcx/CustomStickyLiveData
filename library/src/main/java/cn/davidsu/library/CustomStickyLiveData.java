 ...
 * <p>
 * 自定义一个可控制是否需要粘性的LiveData   默认无粘性
 * <p>
 * LiveData的自带粘性特性导致当LiveData为静态全局时容易引起很多问题,很烦
 */

public class CustomStickyLiveData<T> {

    private static final String TAG = CustomStickyLiveData.class.getSimpleName();
    ...
    public void observeForever(@NonNull Observer<T> observer, boolean isSticky) {
        ...
        if (existing != null && existing instanceof CustomStickyLiveData.LifecycleBoundObserver) {
            throw new IllegalArgumentException("Cannot add the same observer"
                    + " with different lifecycles");
        }
        if (existing != null) {
